%this class implements a 2D Dirichlet boundary condition
...
classdef vectorDirichletBC < handle
    ...
    methods
        ...
        function [Res, Jac] = apply(Res, Jac)
            ...
            for j=1:2 %(1 and 2 are disp dofs, 3 is pressure dof)
                if (obj.nodeSet(2*(n-1)+j) == 1)
                    %get dof number for ifix
                    m = 2*(n-1)+j;
                    BC_vector_plus(m) = 0;
                    BC_vector_minus(m) = 1;
                    Res(m) = obj.valuesSet(2*(n-1)+j);
                end
            end
